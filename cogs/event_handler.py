...
class EventHandler(commands.Cog):
    ...
    def _validate_config(self):
        ...
        logger.info(f"EventHandler configured: Create on {self._day_name(self.create_day)} at {self.create_hour}:00, "
                    f"Delete on {self._day_name(self.delete_day)} at {self.delete_hour}:00 ({self.timezone})")
    ...
    async def _cleanup_stale_events(self):
        ...
        try:
            ...
            if not guild:
                logger.warning(f"Guild {self.guild_id} not found during cleanup")
                return
            ...
            for event_data in active_events:
                ...
                try:
                    ...
                    # Event doesn't exist on Discord, mark as deleted in database
    ...
    def _should_create_event(self, now: datetime, current_time_key: str) -> bool:
        ...
        return (now.weekday() == self.create_day and
                now.hour == self.create_hour and
                self._last_create_check != current_time_key)

    def _should_delete_event(self, now: datetime, current_time_key: str) -> bool:
        """Check if we should delete an event now."""
        return (now.weekday() == self.delete_day and
                now.hour == self.delete_hour and
                self._last_delete_check != current_time_key)
    ...
    async def before_check_schedule(self):
        ...
        logger.info(f"Event schedule check started (interval: {self.check_interval} minutes)")
    ...
    async def _create_weekly_event(self):
        ...
        try:
            ...
            guild = self.bot.get_guild(self.guild_id)
            if not guild:
                logger.error(f"Guild {self.guild_id} not found for event creation")
                return
    ...
    def _calculate_next_sunday(self) -> Optional[datetime]:
        ...
        try:
            ...
            if days_until_sunday == 0:  # If today is Sunday
                ...
                event_time_today = now.replace(hour=self.event_time_hour, minute=self.event_time_minute,
    ...
    def _create_event_description(self) -> str:
        ...
        description = (
            "**Please select \"Interested\" if you intend to participate**\n\n"
    ...
    async def _send_event_notification(self, guild: discord.Guild, event: discord.ScheduledEvent):
        ...
        try:
            ...
            if not notification_channel:
                logger.warning(f"Notification channel {self.event_notification_channel_id} not found")
                return

            role = guild.get_role(self.event_notification_role_id)
            if not role:
                logger.warning(f"Notification role {self.event_notification_role_id} not found")
            ...
            logger.info(f"Sent event notification to {notification_channel.name}")
    ...
    async def _delete_old_event(self):
        ...
        try:
            ...
            guild = self.bot.get_guild(self.guild_id)
            if not guild:
                logger.error(f"Guild {self.guild_id} not found for event deletion")
                return
            ...
            try:
                ...
                self.db.update_event_participants(
                    ...
                    [user['name'] for user in interested_users]
                )
    ...
    async def on_ready(self):
        ...
        logger.info(f"Event schedule: Create on {self._day_name(self.create_day)} at {self.create_hour}:00, "
                    f"Delete on {self._day_name(self.delete_day)} at {self.delete_hour}:00 ({self.timezone})")
